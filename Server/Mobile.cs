...
namespace Server
{
    ...
    public class Mobile : IEntity, IHued, IComparable<Mobile>, ISerializable, ISpawnable
    {
        ...
        {
            get { return m_NextSkillTimeTick; }
            set { m_NextSkillTimeTick = value; }
        }
        ...
        private class SimplePrompt : Prompt
        {
            ...
            public SimplePrompt(PromptCallback callback) : this(callback, false) { }
                        ...
                        )
                        {
                            if (p == null)
                            {
                                ...
                                IEntity src = new Entity(
                                    Serial.Zero,
                                    new Point3D(m_Location.X, m_Location.Y, m_Location.Z + 12),
                                    m_Map
                                );
                                IEntity trg;
                                ...
                                p = Packet.Acquire(
                                    new DragEffect(src, trg, item.ItemID, item.Hue, item.Amount)
                                );
                            }
        ...
        public virtual bool CanBeRenamedBy(Mobile from)
        {
            return @from.AccessLevel >= AccessLevel.GameMaster
                && @from.m_AccessLevel > m_AccessLevel;
        }
        ...
        private static int[] m_InvalidBodies = new int[] { 32, 95, 156, 197, 198, };
                ...
                if (map != null)
                {
                        ...
                        foreach (object o in eable)
                        {
                            ...
                            else if (o != this && o is Mobile)
                            {
                                ...
                                if (
                                    m.m_NetState != null
                                    && (
                                        isTeleport && (!m.m_NetState.HighSeas || !m_NoMoveHS)
                                        || !inOldRange
                                    )
                                    && m.CanSee(this)
                                )
                    ...
                    else
                    {
                        ...
                        foreach (NetState ns in eable)
                        {
                            if (
                                (
                                    isTeleport && (!ns.HighSeas || !m_NoMoveHS)
        ...
        public virtual bool OnDragDrop(Mobile from, Item dropped)
        {
            if (from == this)
            {
                ...
                from.Player && this.Player && from.Alive && this.Alive && from.InRange(Location, 2)
            )
            {
                NetState ourState = m_NetState;
