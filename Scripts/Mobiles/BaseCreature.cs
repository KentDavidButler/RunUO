...
namespace Server.Mobiles
{
    ...
    public partial class BaseCreature : Mobile
    {
        ...
        public virtual bool IsEnemy(Mobile m)
        {
            ...
            if (
                FightMode == FightMode.Evil && m.Karma < 0
                || c.FightMode == FightMode.Evil && Karma < 0
            )
                return true;
        ...
        public virtual double GetControlChance(Mobile m, bool useBaseSkill)
        {
            ...
            int taming = (int)(
                (
                    useBaseSkill
                        ? m.Skills[SkillName.AnimalTaming].Base
                        : m.Skills[SkillName.AnimalTaming].Value
                ) * 10
            );
            int lore = (int)(
                (
                    useBaseSkill
                        ? m.Skills[SkillName.AnimalLore].Base
                        : m.Skills[SkillName.AnimalLore].Value
                ) * 10
            );
            int bonus = 0,
                chance = 700;
        ...
        {
            get { return this.Body.IsMonster || this.IsBonded; }
        }
        ...
        */
        ...
        public virtual void OnCarve(Mobile from, Corpse corpse, Item with)
        {
            ...
            if (
                (feathers == 0 && wool == 0 && meat == 0 && hides == 0)
                || Summoned
                || IsBonded
                || corpse.Animated
            )
                        ...
                        if (IsBondable && !IsBonded)
                        {
                            ...
                            if (master != null && master == from) //So friends can't start the bonding process
                            {
                                if (
                                    ...
                                    || master.Skills[SkillName.AnimalTaming].Base >= m_dMinTameSkill
                                    || OverrideBondingReqs()
                                    || (
                                        master.Skills[SkillName.AnimalTaming].Value
                                        >= m_dMinTameSkill
                                    )
                                )
                                {
        ...
        public virtual bool CheckTeach(SkillName skill, Mobile from)
        {
            ...
            if (
                ...
                && from.Skills[SkillName.Hiding].Base < Stealth.HidingRequirement
            )
                return false;
            ...
            if (
                ...
                && (
                    from.Skills[SkillName.Lockpicking].Base < 50.0
                    ...
                    )
                    {
                        ...
                        list.Add(
                            new TeachEntry(
                                ...
                                this,
                                from,
                                (toTeach > theirSkill.BaseFixedPoint)
                            )
                        );
                    }
                }
        ...
        public override bool HandlesOnSpeech(Mobile from)
        {
            ...
            return m_AI != null
                && m_AI.HandlesOnSpeech(@from)
                && @from.InRange(this, m_iRangePerception);
        }
        ...
        */
        ...
        public virtual void CheckedAnimate(
            int action,
            int frameCount,
            int repeatCount,
            bool forward,
        ...
        public override void OnMovement(Mobile m, Point3D oldLocation)
        {
            ...
            GuardedRegion guardedRegion = (GuardedRegion)
                this.Region.GetRegion(typeof(GuardedRegion));
        ...
        {
            if (
                target is BaseCreature && ((BaseCreature)target).IsInvulnerable
        ...
        public void DoMining()
        {
            ...
            HarvestResource resource = system.MutateResource(
                this,
                null,
                def,
                map,
            ...
            if (
                skillBase >= resource.ReqSkill
                && CheckSkill(def.Skill, resource.MinSkill, resource.MaxSkill)
            )
            {
        ...
        private void GetHarvestOffset(Direction d, ref int x, ref int y, int maxDistance = 0)
        {
            int distance = 1;
            ...
            if (maxDistance < 1)
            {
                distance = Utility.Random(2, maxDistance);
            }
        ...
        public virtual void OnThink()
        {
            ...
            if (CanBreath && DateTime.Now >= m_NextBreathTime) // tested: controlled dragons do breath fire, what about summoned skeletal dragons?
            {
                ...
                {
                    if (
                        DateTime.Now - m_NextBreathTime < TimeSpan.FromSeconds(30)
                        && Utility.RandomBool()
                    )
                    {
                        BreathStart(target);
            ...
            if (ReturnsToHome && this.IsSpawnerBound() && !this.InRange(Home, RangeHome))
            {
                ...
                {
                    m_FailedReturnHome = !this.Move(GetDirectionTo(Home.X, Home.Y))
                        ? m_FailedReturnHome + 1
                        : 0;
        ...
        public String NPCRandomSpeech(
            bool isFemale,
            bool isVendor,
            bool isFunny,
            bool isAggressive,
            List<int> optionalStatements = null
        )
        {
        ...
        private bool IsSpawnerBound()
        {
            if (Map != null && Map != Map.Internal)
            {
                if (FightMode != FightMode.None && RangeHome >= 0)
                {
                    ...
                    {
                        if (
                            Spawner != null && Spawner is Spawner && (Spawner as Spawner).Map == Map
                        )
                        {
                            return true;
                        }
        ...
        public override void OnSectorDeactivate()
        {
            ...
            {
                Timer.DelayCall(
                    TimeSpan.FromSeconds(Utility.Random(45) + 15),
                    new TimerCallback(GoHome_Callback)
                );
                    ...
                    if (c.IsDeadPet)
                    {
                        ...
                        )
                        {
                            if (c.OwnerAbandonTime == DateTime.MinValue)
                            {
                                ...
                                (c.OwnerAbandonTime + c.BondingAbandonDelay) <= DateTime.UtcNow
                            )
                            {
                                lock (toRemove)
                                    toRemove.Add(c);
